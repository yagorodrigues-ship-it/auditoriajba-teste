                        ...
                        with col_act1:
                            if st.button("🚨 Abrir 2ª Contagem para Almoxarife", type="primary", use_container_width=True):
                                if not justificativa_adm.strip():
                                    ...
                                    cursor = conn.cursor()
                                    cursor.execute("UPDATE contagens SET fase_contagem = '2a Contagem', qtd_contada = 0, diferenca = 0 - qtd_sistema, observacao = %s WHERE id = %s", (obs_final, row_item_alvo['id']))
